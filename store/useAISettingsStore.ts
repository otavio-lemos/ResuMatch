...
import { getAtsUISkill } from '@/lib/get-skill';
...
const defaultAISettings: AISettings = {
  ...
  model: 'gemini-flash-latest', temperature: 0.7, topP: 0.9, topK: 40, maxTokens: 16384, frequencyPenalty: 0, presencePenalty: 0, timeout: 120000,
};

const defaultImportAISettings: AISettings = {
  provider: 'gemini', apiKey: '', baseUrl: 'https://generativelanguage.googleapis.com/v1beta/',
  model: 'gemini-flash-latest', temperature: 0.1, topP: 0.9, topK: 40, maxTokens: 16384, frequencyPenalty: 0, presencePenalty: 0, timeout: 120000,
};

const defaultImportPrompt = "Analise o currículo e extraia todas as informações relevantes.";
const defaultSummaryPrompt = "Gere um resumo profissional seguindo o SKILL.md. Use 3-5 parágrafos por seção, 67-94 caracteres por parágrafo.";
...
Você receberá o texto bruto do bullet a ser reescrito.
...
export const useAISettingsStore = create<AIConfigState>()(
  persist(
    (set, get) => ({
      ...
      setPrimaryAI: (settings) => set((state) => ({ primaryAI: { ...state.primaryAI, ...settings } })),
